...
The Lexical Environment is associated with some specific syntactic structure of ECMAScript code such as a FunctionDeclaration, a BlockStatement, or a Catch clause of a TryStatement and a new Lexical Environment is created each time such code is evaluated.
...
Lexical Environment, as a usual object, stays in memory until there are references to it.
...
#### Global environment
The *global environment* is associated with the whole script.

It has next differences with "usual" Lexical Environment:
1. Its Environment Record stores reference to global object, as a value of "this" identifier. All variables declared in the script are assigned to the global object.
2. And its Outer Lexical Environment reference points to null.

#### Function environment
A *function environment* corresponds to the invocation of an ECMAScript function object.

The differences with "usual" Lexical Environment are:
...
2. A function environment takes reference to the outer Lexical Environment from a Function hidden property \[\[Environment]].

All functions remember a Lexical Environment in which they were **created** (declared and initialized). They keeps the reference to the Lexical Environment in the hidden property named \[\[Environment]].
